...
const [form, setForm] = useState({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
const payload = { ...form, blanks: form.blanks }; // language tetap termasuk di form
const res = await api.post(`/admin/modules/${moduleId}/challenges`, payload);

      console.log("Challenge berhasil dibuat:", res.data);
      navigate(`/admin/modules/${moduleId}/challenges`);
    } catch (err) {
      console.error(err.response?.data || err);
      alert("Gagal membuat challenge. Cek console untuk detail.");
    }
  };

  return (
    <div className="max-w-xl p-6 mx-auto mt-10 bg-white rounded-md shadow-md">
      <h2 className="mb-6 text-2xl font-semibold text-gray-800">Tambah Challenge C++ Rumpang</h2>
