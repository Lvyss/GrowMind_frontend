...
import { getUserModule } from "../api/api"; // pakai function dari api.js
...
export default function ModuleShow() {
  ...
  useEffect(() => {
    const fetchModule = async () => {
      try {
        const data = await getUserModule(slug); // <-- gunakan fungsi dari api.js
        setModule(data || null);
      } catch (err) {
  ...
  if (!module) return <div className="p-6 text-gray-500">Loading module...</div>;
  ...
  return (
    <div className="max-w-4xl p-8 mx-auto space-y-6 bg-white rounded-lg shadow-md">
      ...
      {/* Content */}
      <div className="mt-4 prose break-words prose-slate max-w-none">
